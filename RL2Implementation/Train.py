...
from speakers import speaker_output
...
parser.add_argument('--entropyCoef', type=float, default=1e-5,
...
def load_next_opponent():
    ...
    #return speakers
    return random_opponent_id

def eval_load_next_opponent():
    random_opponent_id = random.randint(6, 10)
    #speakers = DDPG(3, 3, 22, 64, 0.01)
    #speakers.load_params(torch.load('agents/agent' + str(random_opponent_id) + '/params_25000.pt')['agent_params'][0])
    #return speakers
    return random_opponent_id
...
def train(args):
    f = open("output_generalize_"+str(args.seed), "a")
    f.write("Training episode, Rewards\n")
    f.close()
    f = open("output_train_" + str(args.seed), "a")
    f.write("Training episode, Rewards\n")
    f.close()

    updates_pre_episode = math.ceil(args.max_episode_length / args.thread_update_length)
    ...
    while episode_number < 800*updates_pre_episode:
        for idx in range(args.num_threads):
            ...
            if a['timestep_left'] == 0:
                ...
                if episode_number % args.trial_length == 0:
                    ...
                    opponent_ids = [load_next_opponent() for idx in range(args.num_threads)]
        ...
        for idx in range(args.num_threads):
            loss_elements, hidden_states = get_rollouts(environment[idx], agent, args, hidden_elements[idx],
                         concat_elements[idx], opponent_ids[idx], args.thread_update_length)
        ...
        episode_number += 1
        if episode_number % (args.eval_freq*updates_pre_episode) == 0:
...
def get_rollouts(environment, agent, args, hidden_states, concat_elements, opponent, rollout_length=10, timesteps_left=25):
    ...
    for a in range(min(rollout_length,timesteps_left)):
        ...
        opponent_acts = speaker_output(opponent, states[0])
    ...
    while episode_number < 50:
        ...
        for a in range(args.trial_length):
            ...
            while not done:
                ...
                speaker_out_list = speaker_output(oppo_number, opponent_input_tensor)
            ...
            f = open("output_train_"+str(args.seed), "a")
            f.write(str(num_eps)+","+str(episode_reward) + "\n")
            num_eps += 1
            returns_obtained.append(episode_reward)
            f.close()

        oppo_number = eval_load_next_opponent()
        hidden_states = (torch.zeros((1, 1, args.hidden_dim)), torch.zeros((1, 1, args.hidden_dim)))
        total_reward = 0
        num_eps = 2 * episode_number
        for a in range(args.trial_length):
            ...
            while not done:
                ...
                policy, _, hidden_states = agent.step(tensor_state[1], action_adds, reward_adds, done_adds,
                                                      hidden_states)
                listener_act = onehot_from_logits(policy)
                # listener_act = agent.act(policy)
                # speaker_out_list = oppo_model.step(opponent_input_tensor, explore=False)
